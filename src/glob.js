...
//
const cleanGlobPath = WIN32
  ? (path, volumeNameLen) => {
      // (prefixLen int, cleaned string)
      ...
      if (vollen == path.length && path.length == 2) {
        // C:
...
export function match(pattern, name) {
  Pattern: while (pattern.length > 0) {
    ...
    if (star) {
      ...
      for (
        let i = 0;
        i < name.length && name.charCodeAt(i) != DIRSEP_BYTE;
        i++
      ) {
  ...
  while (chunk.length > 0) {
    ...
    switch (chunk.charCodeAt(0)) {
      case 0x5b: {
        ...
        log(
          '"[" s reduced to %o, chunk reduced to %o, r = 0x%s',
          s,
          chunk,
          r.toString(16)
        );
      ...
      default:
        if (chunk.charCodeAt(0) != s.charCodeAt(0)) {
          log(
            '[def] chunk[0] != s[0] (%o != %o) (0x%s != 0x%s) -- return',
            chunk[0],
            s[0],
            chunk.charCodeAt(0).toString(16),
