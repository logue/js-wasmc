...
export async function build(c, allmodules /* = c.config.modules*/) {
  ...
  if (
    !c.force &&
    !c.config.didConfigure &&
    !ninjaBotInstances.has(projectdir)
  ) {
  ...
  if (dirtyWasmMods.length > 0) {
    dlog(
      '>> build/ninja',
      dirtyWasmMods.map(m => m.name)
    );
    let targets = dirtyWasmMods.map(m =>
      Path.join(c.config.builddir, m.wasmfile)
    );
  ...
  if (updatedmods.length > 0) {
    ...
    (c.watch ? c.logImportant : c.log)(
      'Built %s in %s',
      names,
      fmtduration(monotime() - startTime)
    );
  } else if (c.watch) {
...
export function buildIncrementally(c) {
  return new Promise((_resolve, endIncrBuild) => {
    ...
    watchConfigFile();
    rebuild(c.config.modules, /* isFirstBuild */ true);
...
async function packagemod(c, m, didBuild, jsSourcesChanged) {
  ...
  if (didBuild && !jsSourcesChanged) {
    ...
    let wasmBufOut = {};
    let apiHashDiff = await compareWasmAPIHash(
      apihashfile,
      emccwasmfile,
      wasmBufOut
    );
  ...
  let packageOptions = {
    ...
    wasmfile: m.embed
      ? null
      : Path.relative(Path.dirname(outfilejs), outfilewasm),
    embed: m.embed,
    esmod: m.format == 'es',
  ...
  if (m.sourceMap) {
    if (m.sourceMap == 'inline') {
      ...
      packageOptions.sourcemapFile = Path.relative(
        Path.dirname(packageOptions.outfile),
        m.sourceMap
      );
  ...
  if (
    !m.embed &&
    emccwasmfile != outfilewasm &&
    (didBuild || !fs.existsSync(outfilewasm))
  ) {
...
function compareWasmAPIHash(apihashfile, emccwasmfile, wasmBufOut) {
  // Promise<identical:bool>
  return new Promise((resolve, reject) => {
    ...
    let wasmbuf = fs.promises.readFile(emccwasmfile);
    wasmBufOut.wasmbuf = wasmbuf;
    let apiHash = hashWasmAPI(wasmbuf);
    let existingApiHash = existingApiHashP;
    // dlog({
...
function writeWasmAPIHashFile(apihashfile, emccwasmfile) {
  // Promise<identical:bool>
  return fs.promises
    .readFile(emccwasmfile)
    .then(hashWasmAPI)
...
function modsToBuild(c, allmodules) {
  ...
  dirtyWasmModsP = new Promise(
    (dirtyWasmMods_resolve, dirtyWasmMods_reject) => {
...
function checkModSource(c, m) {
  ...
  r.sourcesP = new Promise((sources_resolve, sources_reject) => {
    r.wasmUpToDateP = new Promise(
      (wasmUpToDate_resolve, wasmUpToDate_reject) => {
